    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['people:athletes:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['people:athletes:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['people:athletes:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['people:athletes:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listAthletes, getAthletes, delAthletes, addAthletes, updateAthletes } from "@/api/people/athletes";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getAthletes(athleteId).then(response => {
        this.loading = false;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.athleteId != null) {
            updateAthletes(this.form).then(response => {
            ...
            addAthletes(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除运动员管理 编号为"' + athleteIds + '"的数据项？').then(() => {
        this.loading = true;
        return delAthletes(athleteIds);
      }).then(() => {
        this.loading = false;
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('people/athletes/export', {
        ...this.queryParams
      }, `athletes_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
